...
#             target_param.data.copy_(tau * param.data + (1.0 - tau) * target_param.data)
    ...
    def update_epsilon(self):
        ...
        if self.t < middle:
            ...
            self.epsilon = high + slope * (self.t - start)
        elif self.t < end:
            slope = (low - medium) / (end - middle)
            self.epsilon = medium + slope * (self.t - middle)
        else:
            self.epsilon = low
    ...
    #     self.t += 1  # increment the timestep after updating epsilon
        ...
        if self.t % 50 == 0 and self.t!=0:  # Print only every 50 steps
            print(f"Step: {self.t}, Epsilon updated to: {self.epsilon}")
...
#         print(newstate.shape)
        ...
        #update t after one iteration,t keeps to be zero utill the buffer is full and train process starts. 
        self.t+=1
