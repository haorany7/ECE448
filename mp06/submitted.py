...
def minimax(side, board, flags, depth):
    ...
    moveTree = {}
    if side: # minimize (player 1)
        bestValue = float('inf')
        bestMoves = []
        for move in generateMoves(side, board, flags):
            newSide, newBoard, newFlags = makeMove(side, board, move[0], move[1], flags, move[2])
            value, subMoves, subTree = minimax(newSide, newBoard, newFlags, depth-1)
            if value < bestValue:
                bestValue = value
                bestMoves = [move] + subMoves
            moveTree[encode(*move)]=subTree
    else: # maximize (player 2)
        bestValue = float('-inf')
        bestMoves = []
        for move in generateMoves(side, board, flags):
            newSide, newBoard, newFlags = makeMove(side, board, move[0], move[1], flags, move[2])
    ...
    return bestValue, bestMoves, moveTree
...
def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if side: # minimize (player 1)
        ...
        for move in generateMoves(side, board, flags):
            newSide, newBoard, newFlags = makeMove(side, board, move[0], move[1], flags, move[2])
            value, subMoves, subTree = alphabeta(newSide, newBoard, newFlags, depth-1, alpha, beta)
            if value < bestValue:
                bestValue = value
                bestMoves = [move] + subMoves
            moveTree[encode(*move)] = subTree
